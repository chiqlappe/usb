...
static void rp2040_configure_pio_usb(void) {
  ...
  Serial1.println("Core1 setup to run TinyUSB host with pio-usb");
  ...
  if (cpu_hz % 12000000UL) {
    ...
    Serial1.printf("Error: CPU Clock = %lu, PIO USB require CPU clock must be multiple of 12 Mhz\r\n", cpu_hz);
    Serial1.printf("Change your CPU Clock to 12*n Mhz in Menu->CPU Speed \r\n");
    while (1) {
      delay(1);
    }
  }
